...
variable_features <- readRDS("variable_genes.rds")  
...
seurat_object =
  ...
  mutate(seurat = map(file, ~ {            #purr::map(vector, function)
    seu =
      LoadH5Seurat(.x, assay="SCT")[variable_features,] |>
...
# seurat_object |> saveRDS("Output/seurat_object.rds")
#
# seurat_object = readRDS("Output/seurat_object.rds")
...
immune.anchors |> saveRDS(paste0("Output/immune.anchors_", filename_part, ".rds"))
...
immune.combined |> saveRDS(paste0("Output/immune.combined_", filename_part, ".rds"))
